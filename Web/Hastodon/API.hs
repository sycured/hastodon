...
getHastodonResponseJSON path client =
  mkHastodonRequest path client >>= httpJSONEither
...
mkHastodonClient
  :: String -> String -> String -> String -> String -> IO (Maybe HastodonClient)
mkHastodonClient clientId clientSecret username password host = do
  ...
  case oauthRes of
    ...
    Right oauthData ->
      return $ Just $ HastodonClient host (accessToken oauthData)

getAccountById
  :: HastodonClient -> AccountId -> IO (Either JSONException Account)
...
getFollowersWithOption
  ...
  -> RangeOption
  -> AccountId
  -> IO (Either JSONException [Account])
...
getAccountStatuses
  :: HastodonClient -> AccountId -> IO (Either JSONException [Status])
getAccountStatuses client = getAccountStatusesWithOption client mempty

getRelationships
  :: HastodonClient -> [AccountId] -> IO (Either JSONException [Relationship])
getRelationships client ids = do
  let intIds = map (show) ids
  let params = foldl
        (\x y -> x ++ (if x == "" then "?" else "&") ++ "id%5b%5d=" ++ y)
        ""
        intIds
...
getSearchedAccountsWithOption client opt query = do
  res <- getHastodonResponseJSONWithOption (optionAsQuery opt)
                                           (pSearchAccounts ++ "?q=" ++ query)
                                           client
  return (getResponseBody res :: Either JSONException [Account])

getSearchedAccounts
  :: HastodonClient -> String -> IO (Either JSONException [Account])
getSearchedAccounts client = getSearchedAccountsWithOption client mempty

postFollow
  :: HastodonClient -> AccountId -> IO (Either JSONException Relationship)
postFollow client id = do
  res <- postAndGetHastodonResponseJSON (replace ":id" (show id) pFollow)
                                        []
                                        client
  return (getResponseBody res :: Either JSONException Relationship)
...
postUnfollow client id = do
  res <- postAndGetHastodonResponseJSON (replace ":id" (show id) pUnfollow)
                                        []
                                        client
  return (getResponseBody res :: Either JSONException Relationship)
...
postBlock client id = do
  res <- postAndGetHastodonResponseJSON (replace ":id" (show id) pBlock)
                                        []
                                        client
  return (getResponseBody res :: Either JSONException Relationship)
...
postUnblock client id = do
  res <- postAndGetHastodonResponseJSON (replace ":id" (show id) pUnblock)
                                        []
                                        client
  return (getResponseBody res :: Either JSONException Relationship)
...
postMuteWithOption
  ...
  -> PostMuteOption
  -> AccountId
  -> IO (Either JSONException Relationship)
postMuteWithOption client opt id = do
  let prms = optionAsForm opt
  res <- postAndGetHastodonResponseJSON (replace ":id" (show id) pMute)
                                        prms
                                        client
  return (getResponseBody res :: Either JSONException Relationship)
...
postUnmute client id = do
  res <- postAndGetHastodonResponseJSON (replace ":id" (show id) pUnmute)
                                        []
                                        client
  return (getResponseBody res :: Either JSONException Relationship)
...
postApps host clientName = do
  let reqBody =
        [ (Char8.pack "client_name"  , utf8ToChar8 clientName)
...
getBlocksWithOption
  :: HastodonClient -> RangeOption -> IO (Either JSONException [Account])
...
getFavoritesWithOption
  :: HastodonClient -> RangeOption -> IO (Either JSONException [Status])
...
postAuthorizeRequest client id =
  postAndGetHastodonResult (replace ":id" (show id) pAuthorizeRequest) [] client

postRejectRequest :: HastodonClient -> AccountId -> IO Bool
postRejectRequest client id =
  postAndGetHastodonResult (replace ":id" (show id) pRejectRequest) [] client
...
postMediaFile
  :: HastodonClient -> String -> String -> IO (Either JSONException Attachment)
postMediaFile client filename description = do
  ...
  req <- formDataBody
    [ file { partContentType = Just mimetype }
...
getNotificationsWithOption
  ...
  -> IO (Either JSONException [Notification])
...
getNotificationById
  :: HastodonClient -> NotificationId -> IO (Either JSONException Notification)
getNotificationById client id = do
  res <- getHastodonResponseJSON (replace ":id" (show id) pNotificationById)
                                 client
  return (getResponseBody res :: Either JSONException Notification)
...
getSearchedResults
  :: HastodonClient -> String -> IO (Either JSONException [Results])
...
getSearchedResultsWithOption
  ...
  -> StatusSearchOption
  -> String
  -> IO (Either JSONException [Results])
...
getRebloggedByWithOption client opt id = do
  res <- getHastodonResponseJSONWithOption
    (optionAsQuery opt)
    (replace ":id" (show id) pRebloggedBy)
    client
  return (getResponseBody res :: Either JSONException [Account])

getRebloggedBy
  :: HastodonClient -> StatusId -> IO (Either JSONException [Account])
...
getFavoritedByWithOption client opt id = do
  res <- getHastodonResponseJSONWithOption
    (optionAsQuery opt)
    (replace ":id" (show id) pFavoritedBy)
    client
  return (getResponseBody res :: Either JSONException [Account])
...
postStatusWithMediaIds
  :: HastodonClient -> String -> [MediaId] -> IO (Either JSONException Status)
postStatusWithMediaIds client status mediaIds = do
  let unpackedMediaIds =
        [ (Char8.pack "media_ids[]", (utf8ToChar8 . unMediaId) media)
        | media <- mediaIds
        ] -- Rails array parameter convention?
...
postReblog client id = do
  res <- postAndGetHastodonResponseJSON (replace ":id" (show id) pReblog)
                                        []
                                        client
  return (getResponseBody res :: Either JSONException Status)
...
postUnreblog client id = do
  res <- postAndGetHastodonResponseJSON (replace ":id" (show id) pUnreblog)
                                        []
                                        client
  return (getResponseBody res :: Either JSONException Status)
...
postFavorite client id = do
  res <- postAndGetHastodonResponseJSON (replace ":id" (show id) pFavorite)
                                        []
                                        client
  return (getResponseBody res :: Either JSONException Status)
...
postUnfavorite client id = do
  res <- postAndGetHastodonResponseJSON (replace ":id" (show id) pUnfavorite)
                                        []
                                        client
  return (getResponseBody res :: Either JSONException Status)
...
getHomeTimelineWithOption
  :: HastodonClient -> TimelineOption -> IO (Either JSONException [Status])
...
getTaggedTimeline
  :: HastodonClient -> String -> IO (Either JSONException [Status])
getTaggedTimeline client hashtag = do
  res <- getHastodonResponseJSON (replace ":hashtag" hashtag pTaggedTimeline)
                                 client
  return (getResponseBody res :: Either JSONException [Status])
